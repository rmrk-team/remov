...
describe("Hub",async function (){
  ...
  describe("Transactions",async function (){
    it("Should create RMRK order and emit event createdRMRKOrder",async function (){
      const tx = await hub.connect(addr1).createRMRKOrder(addr1.address, TestUID);
      const receipt = await tx.wait();

      expect(await hub.balanceOf(addr1.address)).to.equal(0);
      receipt.events?.filter((x) => {return x.event == "createdRMRKOrder"});
    });

    it("Should revert create MOVR order", async function(){
      await expect(hub.connect(addr1).createMOVROrder(TestKusamaAddress, TestUID)).to.be.revertedWith("ERC721: owner query for nonexistent token");
    });
    ...
    it("Should conduct a full normal cycle of nft translation", async function (){
      ...
      await hub.connect(owner).fillRMRKOrder(addr1.address, TestUID, TestURI, []);
  ...
  describe("Pay",async function (){
    ...
    it("Should pay and create an order",async function (){
      await hub.connect(owner).switchFee(true);
      await hub.connect(owner).changeServerPayments(ethers.utils.parseEther('1'));
      await hub.connect(owner).changeDeployPayments(ethers.utils.parseEther('1'));
      ...
      await hub.connect(addr1).createRMRKOrder(addr1.address, TestUID,{value: ethers.utils.parseEther('2')});
      
      expect(await ethers.provider.getBalance(bank)).to.equal(ethers.utils.parseEther('1'));
      expect(await ethers.provider.getBalance(receiver)).to.equal(ethers.utils.parseEther('1'));
    });
    ...
    it("Should lose money and get it back",async function (){
      ...
      await hub.connect(addr1).createRMRKOrder(addr1.address, TestUID,{value: ethers.utils.parseEther('3')});
      
      expect(await ethers.provider.getBalance(bank)).to.equal(bank_balance.add(ethers.utils.parseEther('1')));
      expect(await ethers.provider.getBalance(receiver)).to.equal(receiver_balance.add(ethers.utils.parseEther('1')));
      
      expect(await hub.connect(addr1).myLostMoney()).to.equal(ethers.utils.parseEther('1'));

      await hub.connect(addr1).takeMoney();

      expect(await hub.connect(addr1).myLostMoney()).to.equal(ethers.utils.parseEther('0'));
    });
  });
